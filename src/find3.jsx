...
import boardBg from "./assets/background_find2.png";
...
import duaOrang from "./assets/find/find_duaorang.png";
import mobilBox from "./assets/find/find_mobilbox.png";
import pengendaraMotor from "./assets/find/find_pengendaramotor.png";
...
export default function Find3() {
  ...
  const handleObjClick = (obj) => {
    if (showModal) return;
    setTimerActive(false);
    if (obj === "pengendaraMotor") {
      setModalType("win");
      setModalDesc("Pengendara ini Tidak Menggunakan Helm !!!");
      setShowModal(true);
      // Tidak auto-navigate, tunggu user klik ceklis
    } else {
      setModalType("lose");
      setModalDesc("Objek ini Taat pada aturan !!!");
      setShowModal(true);
    }
  };
  ...
  const handleCloseModal = () => {
    ...
    // Jika ini modal menang, lakukan navigasi random
    if (modalType === "win") {
      ...
      if (idx < order.length) {
        ...
        localStorage.setItem("champions_shown", "false");
        navigate("/gamepage", { state: { showChampions: true } }); // <-- Tambahkan state
      }
      return;
    }
        ...
        >
          ...
          >
            ...
            <img
              src={duaOrang}
              alt="Dua Orang"
              className="obj-dua-orang"
              onClick={() => handleObjClick("duaOrang")}
              style={{ cursor: "pointer" }}
              tabIndex={0}
            />
            <img
              src={mobilBox}
              alt="Mobil Box"
              className="obj-mobil-box"
              onClick={() => handleObjClick("mobilBox")}
              style={{ cursor: "pointer" }}
              tabIndex={0}
            />
            <img
              ...
              className="obj-pengendara"
              onClick={() => handleObjClick("pengendaraMotor")}
              style={{ cursor: "pointer" }}
              tabIndex={0}
            />
          </div>
